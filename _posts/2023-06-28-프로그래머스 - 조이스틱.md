...
▶ - 커서를 오른쪽으로 이동 (마지막 위치에서 오른쪽으로 이동하면 첫 번째 문자에 커서)<br><br>
...
def solution(name):
    ...
    for i, char in enumerate(name):
        answer += min(ord(char) - ord('A'), ord('Z') - ord(char) + 1) # 상하 이동의 적은 횟수로 answer 값에 더함
        next = i+1
        # for loop에서 좌우 이동의 최솟값으로 min_move 업데이트
